...
@interface WeiMiAddressEditVC ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UITextViewDelegate, CityViewDelegate>
{
    /**数据源*/
    NSArray *_dataSource;
    NSMutableArray *_dtoData;
    BOOL _save;
...
@implementation WeiMiAddressEditVC
...
- (void)viewDidLoad
{
    ...
    _model.province = _dto.province;
    _model.city = _dto.city;
    _model.county = _dto.county;
    _model.userName = _dto.name;
    _model.userPhone = _dto.tel;
    _model.street = _dto.street;
    _model.addressId = _dto.addressId;
}
...
{
    WeiMiEditAddressRequest *request = [[WeiMiEditAddressRequest alloc] initWithModel:model];
    request.showHUD = YES;
    WS(weakSelf);
    [request startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([result isEqualToString:@"1"]) {
            ...
            [self presentSheet:@"编辑成功"];
            _save = YES;
        }else
        {
            [self presentSheet:@"编辑失败"];
        }
...
//        if (indexPath.row != 2 && indexPath.row != 3) {
    ...
    if (field) {
        ...
        {
            field.text = [NSString stringWithFormat:@"%@,%@,%@", _dto.province, _dto.city, _dto.county];
        }
//        else if (indexPath.row == 3)
//        {
//            field.text = _dto.street;
//        }
//        else if (indexPath.row == 4)
//        {
//            field.placeholder = @"请输入邮编";
//            field.text = _dto.postalcode;
//        }
    }
