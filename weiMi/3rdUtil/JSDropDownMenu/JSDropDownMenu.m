...
#define LeftBGColor  [UIColor colorWithRed:244.0f/255.0f green:244.0f/255.0f blue:244.0f/255.0f alpha:1.0]
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //indicator
        if (i == 2) {//只有当为价格时才显示指示器
            CAShapeLayer *indicator = [self createIndicatorWithColor:[UIColor redColor] andPosition:CGPointMake(titlePosition.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
            [self.layer addSublayer:indicator];
            [tempIndicators addObject:indicator];
        }else
        {
            CAShapeLayer *indicator = [self createIndicatorWithColor:BackColor andPosition:CGPointMake(titlePosition.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
            [self.layer addSublayer:indicator];
            [tempIndicators addObject:indicator];
        }
...
//        }
//    }
    if ([_delegate respondsToSelector:@selector(menu:didSelectMenuAtIndex:)]) {
        
        __weak typeof(self) weakSelf = self;
        [_delegate menu:weakSelf didSelectMenuAtIndex:tapIndex];
    }
...
//        if (tapIndex == 0 || tapIndex == 1 || tapIndex == 2) {
//            return;
//        }
        
        if (tapIndex == _currentSelectedMenudIndex && _show) {//选中的仍然为当前
...
#warning 阻止除了筛选外的view弹出
        if (_currentSelectedMenudIndex == 0 || _currentSelectedMenudIndex == 1 || _currentSelectedMenudIndex == 2) {
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    cell.backgroundColor = LeftBGColor;
